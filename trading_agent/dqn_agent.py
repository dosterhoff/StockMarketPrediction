...
TEST_STEPS_GEN = 500  # How many steps in each epoch for validation          ##Default is None. was also 500

# Initialize random starts within the validation data
VAL_STARTS = None  # random.randint(low=0, high=VAL_SIZE-TEST_STEPS_GEN-1, size=TEST_EPOCHS_GEN)  ##Default is none
...
TURN = 0.01  # Percentage of adjustment to agent's trail       #default is 0.001
COST = 0.7  # Cost of changing financial position               #default is 0.3
...
STEPS = 16 #default is 500.
EPOCHS = 100
WINDOW_LENGTH = 100
ONE_HOT = True  # Agent Position Awareness

GAMMA = 0.95  #default 0.95
LR = .01  #default 0.001   #defines how fast to update weights. If LR is too large, agent may skip the optimal solution. too small, and it may take way too long to find optimum solution
LR_DEC = 0.
TAR_MOD_UP = 0.001
DELTA_CLIP = 1
...
NODES = 16  # Neurons
BATCH_SIZE = 64
MEM_SIZE = 100000 #default is 100000
...
def train(env, dqn):
    ...
    if PLOT_Q_VALUES:
        ...
        plt.plot_q_values(dqn.memory, FOLDER, title)                 #plt.plot_q_values(dqn.q_values_memory, FOLDER, title)
        dqn.q_values_memory = []
